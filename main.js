...
client.on('message_create', async(msg) => {
    if (msg.fromMe) {
        if (msg.body == ".a" && config.pmpermit_enabled == "true" && !msg.to.includes("-")) { // allow and unmute the chat (PMPermit module)

            pmpermit.permitacton(msg.to.split("@")[0])
            var chat = await msg.getChat();
            await chat.unmute(true)
            msg.reply("ප්‍රයිවට් මැසේජ් කරන්න දැන් පුලුවන්.")
            ...
            unmuteDate.setSeconds(Number(unmuteDate.getSeconds()) + Number(config.pmpermit_mutetime));
            await chat.mute(unmuteDate)
            msg.reply(`ඔයාට ප්‍රයිවට් මැසේජ් කරන්න බෑ තව විනාඩි  ${config.pmpermit_mutetime/60} ක් යනකල්.`)

        } else if (msg.body == ".unmute" && !msg.to.includes("-")) { // Unmute an user in pm

            var chat = await msg.getChat();
            await chat.unmute(true)
            msg.reply(`හරි.දැන් ඔයාට මැසේජ් කරන්න පුලුවන්.`)
        ...
        } else if (msg.body.startsWith(".tgraph") && msg.hasQuotedMsg) { // Telegraph Module
            ...
            if (data == "error") {
                quotedMsg.reply(`Error occured while create Telegraph link.`)
            } else {
                quotedMsg.reply(`🔗 *Telegraph Link 👇*\n\n` + "```" + data + "```")
            }
...
app.get('/', (req, res) => {
    res.send('<h1>This server is powered by<a href="https://github.com/navodpro/VirtualWhatsbot"> VirtualWhatsbot</a><br>Copyright by<a href="https://t.me/InfinityJE"> InfinityJE</a></h1><br><a href="https://t.me/Mr_dutugemunu>Contact Dev</a><br><a href=https://t.me/tprojects>Contact Owner</a>')
})
